...
Dota ships its entire balance design as plain-text KV (key-value) data. UCZone
mirrors it as JSON under `<cheat_dir>/assets/data/`. Reading these files is the
most direct way to study how the game's interactions and balance are
structured. Every ability, item, unit and hero is a data row.

This document covers the file layout and the parsing traps. It's patch-
versioned data: the numbers move with balance patches, but the structure is
stable.
...
- A space-separated numeric string is a per-level array. `"9 8 7"` is levels
  1/2/3.
- A `{ "value": ... }` wrapper carries the base value plus metadata. Read
  `value`, ignore the siblings unless you need them.
- `special_bonus_*` keys are talent bonuses; `special_bonus_facet_*` are facet
  bonuses. A `=` prefix (`"=80 =120"`) means the facet overrides the base
...
- Modern abilities often nest the canonical fields (`AbilityCooldown`,
  ...
  `AbilityValues` rather than at the top level. Pudge's Meat Hook cast range
  and Lina's Laguna Blade cooldown are examples. Check both places.

### AbilityBehavior, the behavior bitmask

A `|`-joined flag string (`DOTA_ABILITY_BEHAVIOR_UNIT_TARGET | ...`). It tells
you the ability's category at a glance: `NO_TARGET`, `UNIT_TARGET`, `POINT`,
`TOGGLE`, `PASSIVE`, `CHANNELLED`, `AOE`, `IMMEDIATE`, and so on.
...
> Aghanim's Shard or Scepter. The `IsGrantedByShard` / `HasScepterUpgrade`
> flags on the ability KV are the truth signal. Cross-check Liquipedia before
...
> has to discover them at runtime (`OnModifierCreate`), not from the KV.
...
components; multiple numbered keys are alternative builds). A `*` suffix on a
component name marks the item the recipe upgrades from.

### neutral_items.json

Just the tier assignment. `neutral_tiers.1..5` each list their member items, a
`start_time` and a `craft_cost`. The neutral items' actual stats live in
`items.json` like any other item.
...
> `IsSummoned = "1"`. A hero-grade pet like the Lone Druid Spirit Bear is
...
threat category (enumerate `AbilityBehavior` plus `AbilityUnitDamageType`).
...
single source of truth and re-run it after each patch. Don't hand-edit the
generated table.

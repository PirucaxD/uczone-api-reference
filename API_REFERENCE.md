# UCZone Live-Data API Reference

The functions a script uses to read the current game state: abilities, items,
NPC/unit stats, modifiers, and damage. Every entry was verified against real
script behaviour. Signatures use `name(params) -> return`.

Verified around Dota 2 7.41C (mid-2026). `CItem` extends `CAbility`, so every
`Ability.*` function also works on an item handle unless noted.

Conventions:

- "FINAL" means the value already includes item/buff/talent bonuses. Don't add
  a bonus on top, that double-counts.
- "BASE" means the level value only. You add the matching bonus getter.
- "pcall-guard" means undocumented but present in real builds. Wrap it in
  `pcall` because it's build-variable.
...
|---|---|
| `Ability.GetLevelSpecialValueFor(ability, name, [lvl=-1]) -> number` | A KV special value at the ability's current level (`lvl=-1` auto-resolves). Live and level-aware. This is the correct call for live ability values. |
...
| `Ability.SecondsSinceLastUse(ability) -> number` | Seconds since last use. Returns `-1` if not on cooldown. |
...
| `Ability.IsReady(ability) -> boolean` | `true` if ready. Also `true` for an unlearned ability, so gate on `GetLevel > 0`. |
...
(raw KV value, not level-aware), and `Ability.GetSpecialValueFor(ability, key)`
(works on items too).

### Does not exist, broken, or misleading

- `Ability.GetDamage`: looks live, but it's a static-KV read. For live values
  use `GetLevelSpecialValueFor`.
- `Ability.GetCastRange`: BASE only. Effective range is
  `GetCastRange(a) + NPC.GetCastRangeBonus(npc)`.
- `Ability.GetCooldown`: returns `0` during the cast point (cooldown is set on
  cast completion). For charge abilities it only bumps when all charges are
  spent, so track `GetCurrentCharges` instead.
- `Ability.CanBeExecuted`: returns `-1` for OK. `-1` is truthy in Lua, so
  `if CanBeExecuted(a) then` always passes. Compare `== -1`.
- `Ability.GetLevelSpecialValueFor`: the GitBook page says "WRONG API FIX ME",
  but that's an editorial note, the function works. It returns `0` on a talent
  handle (talent values live in the parent ability's `AbilityValues`).
- `Ability.IsReady`: `true` even for an unlearned ability. Gate on
  `GetLevel > 0`.
- `Ability.GetDirtyButtons`: the doc itself says "Returns we don't know what."
  Avoid.
...
| Signature | Live value returned |
|---|---|
| `NPC.GetItem(npc, name, [isReal=true]) -> CItem\|nil` | Item handle by name. `isReal=true` searches slots 1-6 and neutral; `false` also searches backpack and stash. |
...
| `Item.GetStockCount(item_id, [team]) -> integer` | Shop stock, keyed by numeric item ID, not a handle. |
...
For item usability and cooldown: `CItem` extends `CAbility`, so use the
Ability functions on item handles: `Ability.IsReady(item)`,
`IsCastable(item, mana)`, `GetCooldown(item)`, `GetCurrentCharges(item)`,
`GetManaCost(item)`.

### Gotchas

- An item on its post-stash-move cooldown is reported by `IsItemEnabled` and
  `GetEnableTime`, not by `GetCooldown`.
- `Item.GetStockCount` takes a numeric item ID (from `items.json`), unlike
  every other `Item.*` function.
- `Item.GetPurchaseTime` on an assembled item returns the lowest-index
  component's purchase time.
- `Item.CastsOnPickup` and `Item.CanBeUsedOutOfInventory`: the doc admits it
  has no idea what these do. Untrusted.
...
| `Hero.GetRecentDamage(hero) -> integer` | Damage taken in roughly the last 1s. |
...
### Does not exist, or broken

- `NPC.GetAttackDamage`: does NOT exist, crashes. Use `GetMinDamage`,
  `GetBonusDamage`, `GetTrueDamage`, `GetTrueMaximumDamage`.
- `NPC.GetEvasion`: does NOT exist, and there's no evasion modifier-property
  either. Derive it from items/passives.
- `Entity.GetByIndex`: does NOT exist. Use `Entity.Get(idx)`.
- `NPC.GetAttackTarget` for heroes: does NOT exist. Only
  `Tower.GetAttackTarget(tower)` exists.

### Behavioural gotchas

- BASE-only, must combine: `GetAttackRange` (add `GetAttackRangeBonus`),
  `GetCastRangeBonus` (added to `Ability.GetCastRange`), `GetBaseSpellAmp`
  (add item amp), `GetMinDamage` (use `GetTrueDamage` for the total).
- `GetMoveSpeed` is a STAT, not a velocity. It reads ~285-330 and is non-zero
  while standing still. For a real velocity read
  `Entity.GetField(npc, "m_vecVelocity")` (pcall-guard it). `IsMoving` and
  `IsRunning` are the "actually moving" booleans.
- Already FINAL, never add a bonus: `GetMoveSpeed`, `GetTrueDamage`,
  `GetTrueMaximumDamage`, `GetAttackSpeed`, `GetPhysicalArmorValue`,
  `GetMagicalArmorValue`, the `*DamageMultiplier` getters, `Entity.GetHealth`,
  `GetMaxHealth`, `GetMana`, `GetMaxMana`, `Hero.Get*Total`.
- `Entity.GetAbsOrigin` allocates a Vector each call. Use
  `Entity.GetAbsOriginXYZ` in hot loops.
...
| `NPC.HasAnyModifier(npc, names) -> boolean` | `true` if any of a `string[]` or hash-set (hash-set is faster). |
...
| `NPC.HasState(npc, Enum.ModifierState) -> boolean` | `true` if a state is active (stunned, silenced, hexed, magic-immune, etc.). |
...
| `Modifier.GetDieTime(modifier) -> number` | Gametime the modifier expires. Remaining time is `GetDieTime - GameRules.GetGameTime()`. |
...
- `Modifier.GetModifierAura` always returns `""`. `GetSerialNumber` and
  `GetStringIndex` always return `0`.
- The entire aura modifier API is `@deprecated` (`GetAuraSearchTeam`,
  `GetAuraRadius`, `IsAura`, and so on). `GetProvidedByAura` and
  `IsCurrentlyInAuraRange` are the survivors. Query game state directly
  instead.
- The docs mention `Modifier.GetRemainingTime` but the page never defines it.
  Compute remaining time from `GetDieTime`.

---

## 5. Damage prediction / effective-HP

There's no single `GetEffectiveHP` or `GetTrueDamageVs` engine call.
Effective-HP has to be composed.
...
```
raw damage
  x outgoing-damage modifiers   (attacker's GetModifierProperty)
  x incoming-damage modifiers   (target's GetModifierProperty)
  x armor / resist multiplier   (GetArmorDamageMultiplier / GetMagicalArmorDamageMultiplier)
  - flat block                  (MODIFIER_PROPERTY_AVOID_DAMAGE)
```

Effective HP is `Entity.GetHealth` plus barriers, then divided by the relevant
multiplier. Status resistance scales CC duration, not whether CC lands.

---

## Quick reference: query live vs. look up statically

- Always query live: ability cooldown, charges, mana, level, readiness; item
  presence, charges, enable-time; unit health, mana, armor, attack speed;
  modifier presence, duration, stacks; states; barriers.
...
- Static look-up (no live getter, or the getter is static): ability damage
  tables, talent magnitudes, item geometry. Read these from the KV files, see
